...
config$db_port = 5432
config$db_name = "gpadmin"
config$db_username = "gpadmin"
config$db_table = "experiment_data"
config$db_password = {"pivotal"}
...
con <- dbConnect(
drv, dbname = config$db_name,
host = config$db_host,
port = config$db_port,
user = config$db_username,
password = config$db_password)
rm(config$db_password) # removes the password

# delete table if exists
if(dbExistsTable(con,config$db_table)){
    dbRemoveTable(con, config$db_table)
}
...
re_1 <- new.env()
re_2 <- new.env()
generateData <- new.env()
tempData <- new.env()

#create synthetic data and write it to a DB
generateData$x1 = 11 : (10 + config$n)
generateData$x2 = runif(config$n, 5, 95)
generateData$x3 = rbinom(config$n, 1, 0.5)
generateData$x = data.frame(generateData$x1, generateData$x2, generateData$x3)
generateData$x = scale(generateData$x)
generateData$x = data.frame(1, generateData$x)
dbWriteTable(con, config$db_table, value = generateData$x, append = TRUE, row.names = FALSE)
rm(generateData)

#read from DB
tempData$x = dbGetQuery(con, paste("SELECT * from",config$db_table))

tempData$eps = rnorm(tempData$x[, 1], 0, 1.4) #noise vector
